import { Controller, Post, Body, UseGuards, Req, HttpException, HttpStatus, Get, Param, Delete } from '@nestjs/common';
...
export class PortfoliosController {
    ...
    async getPortfolio(@Req() req: Request, @Param('id') id: string) {
        const user = req['context'].user;
        if(!user) throw new HttpException('User not found', HttpStatus.BAD_REQUEST);
        const portfolio = await this.portfolioService.getPortfolioByIdAndUserId(Number(id), user.id)
