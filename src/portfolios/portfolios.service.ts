...
export class PortfoliosService {
  ...
  async create(dto: CreatePortfolioDto, userId: number) {
    const existedPortfolio = await this.getPortfolioByUserIdAndName(
      dto.name,
      userId,
    );
    if (existedPortfolio)
      throw new HttpException(
        `Portfolio: ${dto.name} already exist`,
  ...
  async deletePortfolio(portfolioId: number, userId: number) {
    const existedPortfolio = await this.getPortfolioByIdAndUserId(
      portfolioId,
      userId,
    );
    if (!existedPortfolio)
      throw new HttpException(
        `You don't have access to delete this portfolio or portfolio was already deleted`,
