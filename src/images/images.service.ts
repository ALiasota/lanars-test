...
export class ImagesService {
    ...
    async create(dto: CreateImageDto, file: any, userId: number) {
        const existedPortfolio = await this.portfolioService.getPortfolioByIdAndUserId(dto.portfolioId, userId)
        if (!existedPortfolio) throw new HttpException(`You can't add image to this portfolio`, HttpStatus.FORBIDDEN);
    ...
    async getImageByIdAndUserId(imageId: number, userId: number) {
        return await this.imageRepository.findOne({ where: { id: imageId, userId } })
    }

    async getAllImages() {
        return await this.imageRepository.findAll({include: {
